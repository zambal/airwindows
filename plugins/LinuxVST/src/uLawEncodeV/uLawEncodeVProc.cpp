/* ========================================
 *  uLawEncodeV - uLawEncodeV.h
 *  Copyright (c) 2016 airwindows, Airwindows uses the MIT license
 * ======================================== */

#ifndef __uLawEncodeV_H
#include "uLawEncodeV.h"
#endif

void uLawEncodeV::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    float* in1  =  inputs[0];
    float* in2  =  inputs[1];
  ...
  while (sampleFrames > 0)
  {
	  ...
	  inputSample = select(abs(inputSample) < 1.18e-23, to_double(fpd) * 1.18e-17, inputSample); 
		...
		Vec4d t = log(1.0+(255.0*abs(inputSample)));
...
void uLawEncodeV::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    double* in1  =  inputs[0];
    double* in2  =  inputs[1];
