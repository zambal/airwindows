...
void ConsoleZChannel::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames)
{
  ...
  fixA[fix_reso] = 4.46570214;
	...
	biquad_1_AL[0] = ((B*B*B*0.9999)+0.0001)*0.499;
	if (biquad_1_AL[0] < 0.0001) biquad_1_AL[0] = 0.0001;

    biquad_1_AL[1] = (C*C*C*29.99)+0.01;
	...
	biquad_2_AL[0] = ((F*F*F*0.9999)+0.0001)*0.499;
	if (biquad_2_AL[0] < 0.0001) biquad_2_AL[0] = 0.0001;

    biquad_2_AL[1] = (G*G*G*29.99)+0.01;
  ...
  while (sampleFrames > 0)
  {
		...
		if(flip) {
			...
			inputSampleL = *in1 * 2.0;
			inputSampleR = *in2 * 2.0;
		...
		temp = (inputSampleL * fixG[fix_a0]) + fixG[fix_sL1];
		fixG[fix_sL1] = (inputSampleL * fixG[fix_a1]) - (temp * fixG[fix_b1]) + fixG[fix_sL2];
		fixG[fix_sL2] = (inputSampleL * fixG[fix_a2]) - (temp * fixG[fix_b2]);
		inputSampleL = temp; //fixed biquad filtering ultrasonics
		temp = (inputSampleR * fixG[fix_a0]) + fixG[fix_sR1];
		fixG[fix_sR1] = (inputSampleR * fixG[fix_a1]) - (temp * fixG[fix_b1]) + fixG[fix_sR2];
		fixG[fix_sR2] = (inputSampleR * fixG[fix_a2]) - (temp * fixG[fix_b2]);
		...
		pd_apply = (fabs(pd_previousSampleL + inputSampleL) / 2.0) * pd_intensity;
		...
		inputSampleL = (drySampleL * (1.0 - pd_apply)) + (inputSampleL * pd_apply);		
		//dry-wet control for intensity also has FM modulation to clean up highs
		pd_previousSampleL = sin(drySampleL);
		//apply the sine while storing previous sample
	
		inputSampleR = sin(inputSampleR);
		//basic distortion factor
		pd_apply = (fabs(pd_previousSampleR + inputSampleR) / 2.0) * pd_intensity;
		//saturate less if previous sample was undistorted and low level, or if it was
		//inverse polarity. Lets through highs and brightness more.
		inputSampleR = (drySampleR * (1.0 - pd_apply)) + (inputSampleR * pd_apply);		
		//dry-wet control for intensity also has FM modulation to clean up highs
		pd_previousSampleR = sin(drySampleR);
		//apply the sine while storing previous sample
